CANYONB_private <- function(date,lat,lon,pres,temp,psal,doxy,param,epres,etemp,epsal,edoxy){
  ...
  inputsdir='code/CONTENT-master/private/' # relative or absolute path to CANYON-B wgts files
  ...
  for (i in (1:noparams)){
    if(paramflag[i]){ # calculate only desired parameters
      ...
      out[['inx']][[paramnames[i]]]=inx[,(5:8)+ioffset];
      if (i==4) {
        ...
        out$inx[[paramnames[i]]]=matrix(outderiv$pCO2*1e-6,nol,4)*out$inx[[paramnames[i]]];
        #rm(outderiv)
      } # end
      rm(inx,inval,ddp)
