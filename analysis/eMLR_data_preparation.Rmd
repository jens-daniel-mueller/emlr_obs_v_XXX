...
```{r zonal_mean_sections_total_cant}
...
cant_3d_section %>%
  ...
  map( ~ p_section_zonal(
    ...
    plot_slabs = "n",
    breaks = seq(0,100,10),
    subtitle_text = paste("1994 estimate:", .x$basin_AIP, .x$data_source)
  ))
...
# changes in disequilibrium
# OceanSODA <- right_join(OceanSODA,
#                        GLODAP %>%
#                          distinct(year, era))
# 
# OceanSODA_glob <- OceanSODA %>%
#   select(year, lat, lon,
#          pCO2, rev_fac, tco2_over_pCO2,
#          grid_area, era) %>%
#   pivot_longer(c(pCO2, rev_fac, tco2_over_pCO2),
#                names_to = "parameter",
#                values_to = "value") %>%
#   mutate(value_area = value * grid_area) %>%
#   drop_na() %>% 
#   group_by(year, era, parameter) %>%
#   summarise(
...
# 
# OceanSODA_glob %>%
#   ggplot(aes(year, value_area_ave, col=era)) +
#   geom_path() +
#   geom_point() +
...
#   ggplot(aes(year, value_area_ave, col="Ocean")) +
#   geom_path() +
#   geom_point() +
#   geom_path(data = co2_atm,
#             aes(year, pCO2, col = "atmosphere")) +
#   geom_point(data = co2_atm,
#             aes(year, pCO2, col = "atmosphere"))
# 
# OceanSODA_glob_era <- OceanSODA_glob %>%
#   group_by(era, parameter) %>%
#   summarise(mean_value = mean(value_area_ave)) %>%
#   ungroup()
# 
# pCO2_ocean_tref <- OceanSODA_glob_era %>% 
#   filter(parameter == "pCO2") %>% 
#   pull(mean_value)
# 
# pCO2_atm_tref <- right_join(co2_atm,
#                             GLODAP %>%
#                               distinct(year, era))
# 
# pCO2_atm_tref <- pCO2_atm_tref %>% 
#   group_by(era) %>% 
...
# 
# disequi_pCO2_tref <-
#   c(0, pCO2_atm_tref - pCO2_ocean_tref)
# 
# disequi_pCO2_change <- 
...
disequi_pCO2_tref <- delta_pco2_annual %>% 
  group_by(era) %>% 
  summarise(mean_delta_pCO2 = mean(delta_pco2)) %>% 
  ungroup() %>% 
...
cant_3d_t2 <- cant_3d %>%
  ...
  mutate(cant_pos =
           (cant_pos_S04 +
              ((median_year - 1994) / 13 * cant_pos_G19)) * (1 + alpha))
...
```{r meridional_sections, eval=FALSE,fig.asp=0.5}
...
```{r meridional_sections_isoneutral_slabs, eval=FALSE, fig.asp=0.6}
