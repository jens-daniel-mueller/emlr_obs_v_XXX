...
```{r cstar_slab_distribution_absolute_pre}
...
```{r cstar_slab_distribution_offset_pre, fig.asp=0.5}
...
```{r correlations_contributions_cstar_pre, fig.asp=0.5}
...
```{r latitudinal_cstar_pattern_pre, fig.asp=0.5}
...
```{r zonal_sections_pre, fig.asp=1}
...
```{r zonal_sections_offset_pre, fig.asp=0.6}
...
GLODAP <- GLODAP %>% 
  mutate(cstar_tref_unadj = cstar_tref) %>% 
  mutate(cstar_tref = if_else(
    str_detect(cruise_expocode, "316N199") &
      basin_AIP == "Indian" &
      data_source == "obs",
    cstar_tref + params_local$cstar_corr_316N199,
    cstar_tref)) %>% 
