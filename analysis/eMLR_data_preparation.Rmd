...
```{r deep_profile_cstar_scatter_filter}
...
cstar_grid <- m_grid_horizontal_coarse(GLODAP)
...
cstar_grid %>%
  ...
  map(
    ~ map +
      geom_tile(
        data = .x,
        aes(lon_grid, lat_grid, fill = cstar_tref),
        col = "grey20"
      ) +
      scale_fill_viridis_c() +
      labs(title = paste(
...
cstar_grid_offset <- cstar_grid %>% 
  ...
  mutate(delta_cstar := !!sym(tref$era[2]) - !!sym(tref$era[1]))


cstar_grid_offset %>%
  filter(data_source == "obs",
         !is.na(delta_cstar)) %>%
  mutate(delta_cstar = cut(delta_cstar,
  ...
  map(
    ~ map +
      geom_tile(
        data = .x,
        aes(lon_grid, lat_grid, fill = delta_cstar),
        col = "grey20"
      ) +
...
p_cstar_tref_absolut <- zonal_sections %>% 
...
p_cstar_tref_offset <- zonal_sections_offset %>%
...
```{r cstar_distributions_cruise, fig.asp=1}
...
```{r cstar_slab_distribution_absolute_tco2}

tco2_grid <- m_grid_horizontal_coarse(GLODAP)
...
tco2_grid %>%
  ...
  map(
    ~ map +
      geom_tile(
        data = .x,
        aes(lon_grid, lat_grid, fill = tco2),
        col = "grey20"
      ) +
      scale_fill_viridis_c() +
      labs(title = paste(
...
```{r tco2_slab_distribution_offset_tco2, fig.asp=0.5}

tco2_grid_offset <- tco2_grid %>% 
  pivot_wider(names_from = era,
              values_from = tco2) %>% 
  mutate(delta_tco2 := !!sym(tref$era[2]) - !!sym(tref$era[1]))


tco2_grid_offset %>%
  filter(data_source == "obs",
         !is.na(delta_tco2)) %>%
  mutate(delta_tco2 = cut(delta_tco2,
                           c(-Inf, -10, -5, -2, 2, 5, 10, Inf))) %>%
  group_split(gamma_slab) %>%
  # head(1) %>%
  map(
    ~ map +
      geom_tile(
        data = .x,
        aes(lon_grid, lat_grid, fill = delta_tco2),
        col = "grey20"
      ) +
      scale_fill_brewer(palette = "RdBu",
                        drop = FALSE,
...
```{r cstar_slab_distribution_absolute_talk}

talk_grid <- m_grid_horizontal_coarse(GLODAP)

talk_grid <- talk_grid %>%
  group_by(lat_grid, lon_grid, gamma_slab, era, data_source, basin_AIP) %>%
  summarise(talk = mean(talk, na.rm = TRUE)*0.5) %>%
  ungroup() %>%
  mutate(gamma_slab = fct_relevel(gamma_slab, sort))
...
talk_grid %>%
  ...
  map(
    ~ map +
      geom_tile(
        data = .x,
        aes(lon_grid, lat_grid, fill = talk),
        col = "grey20"
      ) +
      scale_fill_viridis_c() +
      labs(title = paste(
...
```{r talk_slab_distribution_offset_talk, fig.asp=0.5}

talk_grid_offset <- talk_grid %>% 
  pivot_wider(names_from = era,
              values_from = talk) %>% 
  mutate(delta_talk := !!sym(tref$era[2]) - !!sym(tref$era[1]))


talk_grid_offset %>%
  filter(data_source == "obs",
         !is.na(delta_talk)) %>%
  mutate(delta_talk = cut(delta_talk,
                           c(-Inf, -10, -5, -2, 2, 5, 10, Inf))) %>%
  group_split(gamma_slab) %>%
  # head(1) %>%
  map(
    ~ map +
      geom_tile(
        data = .x,
        aes(lon_grid, lat_grid, fill = delta_talk),
        col = "grey20"
      ) +
      scale_fill_brewer(palette = "RdBu",
                        drop = FALSE,
...
```{r cstar_slab_distribution_absolute_phosphate}

phosphate_grid <- m_grid_horizontal_coarse(GLODAP)

phosphate_grid <- phosphate_grid %>%
  group_by(lat_grid, lon_grid, gamma_slab, era, data_source, basin_AIP) %>%
  summarise(phosphate = mean(phosphate, na.rm = TRUE)*params_local$rCP) %>%
  ungroup() %>%
  mutate(gamma_slab = fct_relevel(gamma_slab, sort))
...
phosphate_grid %>%
  ...
  map(
    ~ map +
      geom_tile(
        data = .x,
        aes(lon_grid, lat_grid, fill = phosphate),
        col = "grey20"
      ) +
      scale_fill_viridis_c() +
      labs(title = paste(
...
```{r phosphate_slab_distribution_offset_phosphate, fig.asp=0.5}

phosphate_grid_offset <- phosphate_grid %>% 
  pivot_wider(names_from = era,
              values_from = phosphate) %>% 
  mutate(delta_phosphate := !!sym(tref$era[2]) - !!sym(tref$era[1]))


phosphate_grid_offset %>%
  filter(data_source == "obs",
         !is.na(delta_phosphate)) %>%
  mutate(delta_phosphate = cut(delta_phosphate,
                           c(-Inf, -10, -5, -2, 2, 5, 10, Inf))) %>%
  group_split(gamma_slab) %>%
  # head(1) %>%
  map(
    ~ map +
      geom_tile(
        data = .x,
        aes(lon_grid, lat_grid, fill = delta_phosphate),
        col = "grey20"
      ) +
      scale_fill_brewer(palette = "RdBu",
                        drop = FALSE,
...
rm(phosphate_grid, phosphate_grid_offset)
