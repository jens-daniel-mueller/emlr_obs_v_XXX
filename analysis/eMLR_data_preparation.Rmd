...
```{r calc_rev_fac_ratio}

tref_mean <- c(sort(tref$median_year)[1]-50 , mean(tref$median_year))

surface <- surface %>% 
  filter(year == mean(tref$median_year))
...
```{r meridional_sections, eval=FALSE}
...
```{r meridional_sections_isoneutral_slabs, fig.asp=0.6, eval=FALSE}
