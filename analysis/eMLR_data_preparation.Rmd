...
unique(glodapv2_xover$parameter)
...
unique(glodapv2_2021_xover$Parameter)
...
```{r merge_GLODAP_expocodes, eval=exist_IO_1990}
...
```{r crossover_analysis_data, eval=exist_IO_1990, fig.asp=1}
...
GLODAP_adjustments_long <- GLODAP_adjustments %>%
  select(cruise_expocode, first_station, last_station,
         tco2_adj, talk_adj, phosphate_adj, nitrate_adj, oxygen_adj, silicate_adj, salinity_adj) %>%
  pivot_longer(tco2_adj:salinity_adj,
               names_to = "parameter",
               values_to = "adjustment")

glodapv2_xover <- left_join(glodapv2_xover,
          GLODAP_adjustments_long %>%
            select(cruise_A = cruise_expocode,
                   parameter,
...
glodapv2_xover <- left_join(glodapv2_xover,
          GLODAP_adjustments_long %>%
            select(cruise_B = cruise_expocode,
                   parameter,
...
glodapv2_xover <- glodapv2_xover  %>% 
  mutate(offset_adj = offset + adjustment_A - adjustment_B)

glodapv2_2021_xover <- left_join(glodapv2_2021_xover,
          GLODAP_adjustments_long %>%
            select(cruise_A = cruise_expocode,
                   parameter,
                   first_station_A = first_station,
...
glodapv2_2021_xover <- glodapv2_2021_xover  %>% 
  mutate(offset_adj = offset + adjustment_A)
...
p_crossover_space <- xover_IO_1990 %>%
  filter(year(dateB) >=2000, year(dateB)<2010) %>% 
  ggplot(aes(y=offset)) +
  geom_hline(yintercept = 0) +
  geom_boxplot() +
  labs(title = "Mean offset 2000s") +
  facet_grid(parameter~., scales = "free_y")

p_crossover_ts + p_crossover_space +
  plot_layout(widths = c(2,1))

# p_crossover_ts_adj <- xover_IO_1990 %>%
...
#         axis.title.x = element_blank())
# 
# p_crossover_space_adj <- xover_IO_1990 %>%
#   filter(year(dateB) >=2000, year(dateB)<2010) %>% 
#   ggplot(aes(y = offset_adj)) +
#   geom_hline(yintercept = 0) +
#   geom_boxplot() +
#   labs(title = "Mean offset 2000s") +
#   facet_grid(parameter~., scales = "free_y")
# 
# p_crossover_ts_adj + p_crossover_space_adj +
#   plot_layout(widths = c(2,1))
# 
# p_adjustment_A_ts <- xover_IO_1990 %>%
#   distinct(cruise_A, cruise_B, first_station_A, first_station_B,
#            dateB, adjustment_A, parameter, RV) %>% 
...
#   distinct(cruise_A, cruise_B, first_station_B, first_station_B,
#            dateB, adjustment_B, parameter, RV) %>% 
...
```{r crossover_analysis_data_glodapv2_xover_only, eval=FALSE}
...
GLODAP_crossover <- GLODAP_crossover %>% 
  select(ends_with("CODE"), ends_with("offset"), parameter)

GLODAP_adjustments_long <- GLODAP_adjustments %>% 
  select(cruise_expocode, first_station, last_station,
         DIC = tco2_adj, ALK = talk_adj, PO4 = phosphate_adj) %>% 
  pivot_longer(DIC:PO4,
               names_to = "parameter",
               values_to = "adjustment")

GLODAP_crossover <- left_join(GLODAP_crossover,
          GLODAP_adjustments_long %>% 
            select(CruiseA_EXPOCODE = cruise_expocode,
                   parameter,
                   first_station_A = first_station,
                   last_station_A = last_station,
                   adjustment_A = adjustment))
...
GLODAP_crossover <- left_join(GLODAP_crossover,
          GLODAP_adjustments_long %>% 
            select(CruiseB_EXPOCODE = cruise_expocode,
                   parameter,
                   first_station_B = first_station,
                   last_station_B = last_station,
                   adjustment_B = adjustment))
...
GLODAP_crossover <- GLODAP_crossover %>% 
  pivot_longer(ends_with("offset"),
               names_to = "space",
               values_to = "offset") %>% 
  mutate(offset_adj = offset + adjustment_A - adjustment_B,
         space = str_remove(space, "_offset"))

GLODAP_crossover_IO_1990_A <- GLODAP_crossover %>%
  filter(CruiseA_EXPOCODE %in% IO_1990_expocodes)

GLODAP_crossover_IO_1990_B <- GLODAP_crossover %>%
  filter(CruiseB_EXPOCODE %in% IO_1990_expocodes)

GLODAP_crossover_IO_1990_B_rev <- GLODAP_crossover_IO_1990_B %>%
  rename(CruiseA_EXPOCODE_back = CruiseA_EXPOCODE,
         CruiseA_EXPOCODE = CruiseB_EXPOCODE,
         adjustment_A_back = adjustment_A,
         adjustment_A = adjustment_B) %>% 
  rename(CruiseB_EXPOCODE = CruiseA_EXPOCODE_back,
         adjustment_B = adjustment_A_back) %>% 
  mutate(offset = -offset,
         offset_adj = -offset_adj)

GLODAP_crossover_IO_1990 <- bind_rows(
  GLODAP_crossover_IO_1990_A,
  GLODAP_crossover_IO_1990_B_rev
)

GLODAP_crossover_IO_1990 <- GLODAP_crossover_IO_1990 %>%
  mutate(dateA = ymd(str_sub(CruiseA_EXPOCODE, 5, 12)),
         dateB = ymd(str_sub(CruiseB_EXPOCODE, 5, 12)))


GLODAP_crossover_IO_1990 <- GLODAP_crossover_IO_1990 %>% 
  mutate(RV = if_else(str_detect(CruiseB_EXPOCODE, "316N"),
                      "316N",
                      "other"))

GLODAP_crossover_IO_1990 %>% 
  count(CruiseA_EXPOCODE, CruiseB_EXPOCODE, parameter, space) %>% 
  filter(n > 1)

p_crossover_ts <- GLODAP_crossover_IO_1990 %>%
  ggplot(aes(dateB, offset, shape = space, col=RV)) +
  geom_hline(yintercept = 0) +
  geom_point() +
  scale_color_brewer(palette = "Set1") +
  facet_grid(parameter~., scales = "free_y") +
...
p_crossover_space <- GLODAP_crossover_IO_1990 %>%
  filter(year(dateB) >=2000, year(dateB)<2010) %>% 
  ggplot(aes(space, offset)) +
  geom_hline(yintercept = 0) +
  geom_boxplot() +
  labs(title = "Mean offset 2000s") +
  facet_grid(parameter~., scales = "free_y")
...
p_crossover_ts_adj <- GLODAP_crossover_IO_1990 %>%
  ggplot(aes(dateB, offset_adj, shape = space, col=RV)) +
  geom_hline(yintercept = 0) +
  geom_point() +
  scale_color_brewer(palette = "Set1") +
  facet_grid(parameter~., scales = "free_y") +
...
p_crossover_space_adj <- GLODAP_crossover_IO_1990 %>%
  filter(year(dateB) >=2000, year(dateB)<2010) %>% 
  ggplot(aes(space, offset_adj)) +
  geom_hline(yintercept = 0) +
  geom_boxplot() +
  labs(title = "Mean offset 2000s") +
  facet_grid(parameter~., scales = "free_y")
...
p_adjustment_A_ts <- GLODAP_crossover_IO_1990 %>%
  distinct(CruiseA_EXPOCODE, CruiseB_EXPOCODE, first_station_A, first_station_B,
           dateB, adjustment_A, parameter, RV) %>% 
  ggplot(aes(dateB, adjustment_A, col=RV)) +
  geom_point() +
  scale_color_brewer(palette = "Set1") +
...
p_adjustment_B_ts <- GLODAP_crossover_IO_1990 %>%
  distinct(CruiseA_EXPOCODE, CruiseB_EXPOCODE, first_station_B, first_station_B,
           dateB, adjustment_B, parameter, RV) %>% 
  ggplot(aes(dateB, adjustment_B, col=RV)) +
  geom_point() +
  scale_color_brewer(palette = "Set1") +
...
GLODAP_IO_1990s <- GLODAP %>%
  filter(between(year, 1989,1999) &
           basin_AIP == "Indian" &
           data_source == "obs") %>% 
  mutate(cruise_expocode_RV = str_sub(cruise_expocode, 1, 7),
...
rm(GLODAP_IO_1990s, GLODAP_IO_1990s_stats, GLODAP_IO_1990s_top5)
...
# calculate surface revelle factor dibution
surface <- surface %>%
  drop_na() %>% 
  mutate(
    rev_fac = buffer(
...
```{r cstar_slab_distribution_absolute}
...
GLODAP_cstar <- GLODAP %>%
  ...
  select(-c(cstar, cstar_tref_delta)) %>%
  filter(data_source == "obs")
...
cstar_grid_offset %>%
  ...
  map(
    ~ map +
      ...
      scale_fill_brewer(name = unique(.x$parameter),
                        palette = "RdBu",
                        drop = FALSE,
                        direction = -1) +
      labs(title = paste(
