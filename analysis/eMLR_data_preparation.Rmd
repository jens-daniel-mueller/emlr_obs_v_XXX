...
if (params$Version_ID == "v_XXX") {
  ...
  # GLODAP_crossover  <-
  # read_csv("data/IO_1990s-2000s_AT_DIC_PO4_mean_cruise_offsets.csv")

  GLODAP_crossover_ALK  <-
    read_csv(paste(path_crossover,
                    "Offset_table_global_ALK.csv",
                   sep = "/"))

  GLODAP_crossover_DIC  <-
    read_csv(paste(path_crossover,
                    "Offset_table_global_DIC.csv",
                   sep = "/"))

  GLODAP_crossover_PO4  <-
    read_csv(paste(path_crossover,
                    "Offset_table_global_PO4.csv",
                   sep = "/"))
  
  GLODAP_crossover <- bind_rows(
    GLODAP_crossover_ALK %>% mutate(parameter = "ALK"),
    GLODAP_crossover_DIC %>% mutate(parameter = "DIC"),
    GLODAP_crossover_PO4 %>% mutate(parameter = "PO4")
  )
...
```{r crossover_analysis_data, eval=exist_IO_1990}
...
```{r crossover_analysis_eyeball, eval=FALSE}
...
GLODAP_crossover <- GLODAP_crossover %>%
  rename(talk = ALK,
         tco2 = DIC,
         phosphate = PO4) %>%
  mutate(phosphate = (phosphate / 100) * 2) %>%
  mutate(
    tco2 = tco2,
    talk = -0.5 * talk,
    phosphate = -params_local$rCP * phosphate +
      params_local$rNP * 0.5 * phosphate,
    cstar = tco2 + talk + phosphate
  ) %>%
  pivot_longer(talk:cstar,
               names_to = "parameter",
               values_to = "value")

GLODAP_crossover %>% 
  group_by(cruise_expocode_RV, parameter) %>% 
...
GLODAP_crossover %>%
  ...
  geom_hline(yintercept = 0) +
  labs(y = "Cruise offsets in C* equivalents [µmol/kg]") +
  geom_point() +
