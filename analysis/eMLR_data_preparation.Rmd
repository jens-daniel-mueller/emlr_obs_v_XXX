...
```{r aou_test, eval=FALSE}

GLODAP %>%
  sample_n(10000) %>%
  mutate(
    temp_cons = gsw_CT_from_t(sal, temp, depth),
    aou_recalc = b_aou(sal, temp, depth, oxygen)
  ) %>%
  ggplot(aes(aou, aou_recalc)) +
  geom_bin2d()
...
if (params_local$data_adjustment == "bulk") {
  ...
  GLODAP_adjustment_stats <-
    bind_rows(
      GLODAP_adj %>%
        count(era, basin) %>%
        mutate(adjustment = "yes"),
      GLODAP_unadj %>%
        filter(data_source == "obs") %>% 
        count(era, basin) %>%
        mutate(adjustment = "no")
    ) %>% 
    mutate(group = "IO_1990")
...
if (params_local$data_adjustment == "cruise") {
  ...
  GLODAP_adjustment_stats <-
    ...
    mutate(adjustment = "no",
           adjustment = if_else(
             is.na(talk_corr_IO_1900) &
               is.na(tco2_corr_IO_1900),
             adjustment,
             "yes"
           )) %>%
    count(era, basin, adjustment) %>% 
    mutate(group = "IO_1990")
...
```{r adjustment_stats_without_adjustments}

if (params_local$data_adjustment == "none") {
...
if (params_local$data_adjustment != "none") {
  ...
  GLODAP_adjustment_stats_temp <- GLODAP %>%
    filter(data_source == "obs") %>%
    mutate(
      adjustment = "no",
      adjustment = if_else(
    ...
    count(era, basin, adjustment) %>%
    mutate(group = "talk_calc")
  
  if (exists("GLODAP_adjustment_stats")) {
    GLODAP_adjustment_stats <-
      bind_rows(GLODAP_adjustment_stats,
                GLODAP_adjustment_stats_temp)
  }
  
  if (!exists("GLODAP_adjustment_stats")) {
    GLODAP_adjustment_stats <- GLODAP_adjustment_stats_temp
  }
  
  rm(GLODAP_adjustment_stats_temp)
...
GLODAP <- GLODAP %>%
  mutate(
    cstar_nitrate = b_cstar_nitrate(tco2, nitrate),
    cstar_phosphate = b_cstar_phosphate(tco2, phosphate),
    cstar_nitrate_talk = b_cstar_nitrate_talk(tco2, nitrate, talk),
    cstar_phosphate_talk = b_cstar_phosphate_talk(tco2, phosphate, talk),
    cstar_talk = b_cstar_talk(tco2, talk)
  )
...
p_prop_prop(df = GLODAP,
            var1 = "cstar_phosphate_talk",
            var2 = "cstar_nitrate_talk") +
  facet_grid(data_source ~ basin_AIP)
...
if (params_local$cstar_nutrient == "phosphate_talk") {
  print(b_cstar_phosphate_talk)
  
  GLODAP <- GLODAP %>%
    rename(cstar = cstar_phosphate_talk) %>%
    select(-starts_with("cstar_"))
}

if (params_local$cstar_nutrient == "nitrate_talk") {
  print(b_cstar_nitrate_talk)
  
  GLODAP <- GLODAP %>%
    rename(cstar = cstar_nitrate_talk) %>%
    select(-starts_with("cstar_"))
}

if (params_local$cstar_nutrient == "talk") {
  print(b_cstar_talk)
  
  GLODAP <- GLODAP %>%
    rename(cstar = cstar_talk) %>%
    select(-starts_with("cstar_"))
}
...
GLODAP_adjustment_stats %>% write_csv(paste(path_version_data,
