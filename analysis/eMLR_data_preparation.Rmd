...
cstar_grid <- cstar_grid %>%
  ...
  summarise(across(c(cstar_tref, tco2, talk, phosphate), ~ mean(.x, na.rm = TRUE))) %>%
  ungroup() %>%
  mutate(
    talk = -0.5 * talk,
    phosphate = -params_local$rCP * phosphate + params_local$rNP *
      0.5 * phosphate
  ) %>% 
mutate(gamma_slab = fct_relevel(gamma_slab, sort))

cstar_grid <- cstar_grid %>% 
  pivot_longer(cstar_tref:phosphate,
               names_to = "parameter",
               values_to = "value")
...
cstar_grid_offset <- cstar_grid_offset %>%
  ...
  mutate(cstar_tref_sum = tco2 + talk + phosphate) %>%
  pivot_longer(cstar_tref:cstar_tref_sum,
               names_to = "parameter",
