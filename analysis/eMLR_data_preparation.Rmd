...
In the following, we fit the full MLR model to the GLODAP data set.

```{r define_full_model}

model <- paste(params_local$MLR_target,
...
cruise_out <- cruise_all %>% 
  filter(rmse > cruise_rmse_max)

GLODAP_out <- GLODAP_augmented %>% 
  filter(cruise %in% cruise_out$cruise)

ggplot() +
  geom_hline(yintercept = cruise_rmse_max) +
...
source("code/CANYON-B-master/CANYONB.R")

# CANYONB(
#   date = as.Date(ymd_hm("2014-12-09 08:45")),
#   lat = 17.6,
#   lon = -24.3,
#   pres = 180,
#   temp = 16,
...
for (i_year in unique(GLODAP$year)) {
...
GLODAP_CB <- GLODAP_CB %>%
  rename(
    "talk_CANYON-B" = "AT",
    "tco2_CANYON-B" = "CT",
    "nitrate_CANYON-B" = "NO3",
    "phosphate_CANYON-B" = "PO4",
    "silicate_CANYON-B" = "SiOH4"
  )

# GLODAP_CB %>%
#   ggplot(aes(`talk_CANYON-B`)) +
#   geom_histogram()
...
for (i_variable in variables) {
  ...
  print(
    ggplot(GLODAP_CB, aes(
      x = !!sym(i_variable),
      y = !!sym(paste0(i_variable, "_CANYON-B"))
    )) +
