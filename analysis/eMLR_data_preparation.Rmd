...
map +
  geom_raster(data = left_join(cstar_SD,
                               GLODAP %>%
                                 distinct(cruise, station, lon, lat)),
              aes(lon, lat, fill = cut(cstar_sd, c(0, 2.5, 5, Inf)))) +
  facet_grid(data_source ~ era) +
  scale_fill_viridis_d(direction = -1, name = "cstar SD") +
  theme(legend.position = "bottom")
...
```{r cstar_slab_distribution, fig.asp=3}
...
map +
  geom_raster(
    data = cstar_grid_offset %>%
      filter(data_source == "obs"),
    aes(lon_grid, lat_grid, fill = delta_cstar)
  ) +
  scale_fill_divergent(mid = "grey70",
                       na.value = "white") +
