...
cant_tref_section <- cant_tref_section %>%
  mutate(delta_cant_anomalous = delta_cant_G19_mean - delta_cant_ss_mean)
...
cant_3d_anom <- cant_3d_anom %>% 
  mutate(delta_cant_emlr_ss = cant_pos - delta_cant_ss)

cant_anom_section <- cant_3d_anom %>%
  select(lon, lat, depth, basin_AIP, data_source, delta_cant_emlr_ss) %>% 
  group_by(data_source) %>%
  nest() %>%
  mutate(section = map(.x = data, ~zonal_mean_section(.x))) %>%
  select(-data) %>%
