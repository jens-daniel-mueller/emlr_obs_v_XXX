...
# GLODAP_augmented_best %>%
#   filter(basin == unique(GLODAP_augmented_best$basin)[2]) %>% 
#   ggplot(aes(lat, .resid)) +
#   geom_bin2d(binwidth = i_binwidth) +
...
# GLODAP_augmented_best_plot <- GLODAP_augmented_best %>%
#   group_by(basin) %>%
#   nest() %>%
#   mutate(
#     plot = map2(
...
#   )
# 
# GLODAP_augmented_best_plot$plot[1]



map(
  unique(GLODAP_augmented_best$basin),
    ~   GLODAP_augmented_best %>% 
    filter(basin == .x) %>% 
    ggplot(aes(lat, .resid)) +
      geom_bin2d(binwidth = i_binwidth) +
