...
Within each basin and slab, linear regression models with lowest summed RMSE across two adjacent (ie compared) eras were selected. The number of selected models is:  

- `r params_local$MLR_number`

Please note, that currently the `lm()` function produces NAs for some predictors. Presumably this happens when two included predictors are collinear. It results in identical rmse values. Regardless, models with the lowest summed rmse are selected, which means that identical models are included in the seletion.

```{r predictor_selection}

# calculate RMSE sum for adjacent eras
lm_all_fitted_wide_eras <- lm_all_fitted_wide  %>%
  select(basin, gamma_slab, model, era, rmse, aic) %>% 
  arrange(era) %>% 
...
lm_all_fitted_wide %>%
  ggplot(aes(as.factor(n_predictors), rmse, col = basin)) +
