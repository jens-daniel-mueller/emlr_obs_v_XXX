...
#     params_local$MLR_predictors_trans
#   )
...
# Cautionary note:
# ols_vif_tol requires a lm object with at least 2 predictors
...
spatial_residual <- GLODAP_augmented_best %>%
  group_by(data_source, gamma_slab, era, lat, lon) %>% 
  summarise(.resid_mean = mean(.resid)) %>% 
  ungroup()
...
spatial_residual_offset <- spatial_residual  %>%
  select(data_source, gamma_slab, era, lat, lon, .resid_mean) %>% 
  arrange(era) %>% 
  group_by(data_source, gamma_slab, lat, lon) %>% 
  mutate(.resid_mean_offset = .resid_mean - lag(.resid_mean)) %>% 
  ungroup() %>% 
  drop_na()
...
GLODAP_augmented_long <- GLODAP_augmented_best %>%
  select(era, basin_AIP, gamma_slab, data_source,
         params_local$MLR_predictors, params_local$MLR_target, .resid) %>% 
  pivot_longer(c(params_local$MLR_predictors,params_local$MLR_target),
               names_to = "predictor",
               values_to = "value")
