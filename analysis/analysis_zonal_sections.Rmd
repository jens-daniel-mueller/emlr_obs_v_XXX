...
```{r dcant_profiles_bias}

dcant_profile_bias <- dcant_profile %>%
  filter(data_source %in% c("mod", "mod_truth")) %>%
  select(depth, basin_AIP, data_source, dcant) %>%
  pivot_wider(names_from = data_source,
              values_from = dcant) %>%
  mutate(dcant_bias = mod - mod_truth)
...
```{r dcant_profiles_decomposition}

dcant_decomposition <-
  bind_rows(
    dcant_profile_bias %>%
      select(depth, basin_AIP, dcant_bias) %>%
      mutate(contribution = "dcant bias"),
    target_profile %>%
      select(depth, basin_AIP, dcant_bias) %>%
