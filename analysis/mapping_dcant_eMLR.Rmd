...
dcant_surface_average <- dcant_surface_average %>% 
  ...
  select(data_source, lat, lon, dcant_surface)
...
dcant_3d <- dcant_3d %>% 
  mutate(dcant_surface = if_else(depth > params_local$depth_min,
                                 NaN,
                                 dcant_surface))

d_min <- params_local$depth_min

dcant_3d <- dcant_3d %>% 
  mutate(dcant = if_else(
    depth <= params_local$depth_min,
    ((d_min - depth)/d_min * dcant_surface) +
      ((1 - (d_min - depth)/d_min) * dcant_eMLR),
    dcant_eMLR)) %>% 
  select(-c(dcant_eMLR, dcant_surface)) %>% 
