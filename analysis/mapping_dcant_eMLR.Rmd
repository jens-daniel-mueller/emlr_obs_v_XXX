...
# delta coefficients for calculation of dcant
lm_best_dcant <-
  read_csv(paste(path_version_data,
                 "lm_best_dcant.csv",
                 sep = ""))
...
```{r merge_model_delta_coeff_predictor_climatology_for_dcant_mapping}
...
# predictors_surface %>% distinct(data_source, depth)
...
dcant_surface <- dcant_surface %>%
  mutate(dcant_atm_clim = (1 / rev_fac) *
           (TCO2 / pCO2) * d_pCO2_tref)

# surface_rev_fac <- dcant_surface

if (params_local$MLR_target %in% c("cstar_phosphate",
                                   "cstar_nitrate",
                                   "cstar_talk",
                                   "cstar_tref_delta")) {
  dcant_surface <- dcant_surface %>%
...
dcant_surface <- full_join(dcant_surface, co2_atm_tref)

co2_atm_tref_2002 <- co2_atm_tref_2002 %>% 
  mutate(pCO2_tref_delta_2002 = 
           pCO2_tref - co2_atm %>% filter(year == 2002) %>% pull(pCO2)) %>% 
  select(era, pCO2_tref_delta_2002)

mean_delta_pCO2_atm <- co2_atm_tref_2002 %>% 
  summarize(mean(pCO2_tref_delta_2002)) %>% 
  pull()

dcant_surface <- dcant_surface %>% 
  mutate(pCO2_tmean = pCO2 + mean_delta_pCO2_atm)
...
dcant_surface <- dcant_surface %>% 
  mutate(
    rev_fac_tmean = buffer(
      flag = 15,
      var1 = TAlk * 1e-6,
      var2 = TCO2_tmean * 1e-6,
      S = sal,
      T = temp,
      P = depth / 10,
      Pt = phosphate * 1e-6,
...
dcant_surface %>%
  mutate(depth = 0,
         delta_rev_fac = rev_fac_tmean - rev_fac) %>% 
  group_split(data_source) %>%
  # head(1) %>%
  map( ~
         p_map_climatology(
...
# calculate cant
dcant_surface <- dcant_surface %>%
  mutate(dcant_disequi_tmean = (1 / rev_fac_tmean) *
           (TCO2_tmean / pCO2_tmean) * d_pCO2_tref_disequi)

# calculate cant
dcant_surface <- dcant_surface %>%
  mutate(dcant_atm_tmean = (1 / rev_fac_tmean) *
           (TCO2_tmean / pCO2_tmean) * d_pCO2_tref)

# surface_rev_fac_disequi <- dcant_surface_disequi

dcant_surface %>%
  mutate(depth = 0,
         delta_dcant = dcant_atm_tmean - dcant_atm_clim) %>%
  group_split(data_source) %>%
  # head(1) %>%
  map(~
        p_map_climatology(
...
dcant_surface %>%
  mutate(depth = 0,
         delta_dcant = dcant_disequi_tmean - dcant_atm_clim) %>%
  group_split(data_source) %>%
  # head(1) %>%
  map(~
        p_map_climatology(
...
dcant_surface <- dcant_surface %>% 
  select(lon:rev_fac, dcant = dcant_atm_tmean)

```

### Sea surface observations

```{r prepare_surface_data, eval=FALSE}
...
```{r calc_long_term_tco2_trends, eval=FALSE}
...
```{r calc_dcant_surface_based_on_surface_observations, fig.asp=1, eval=FALSE}
...
```{r dcant_surface_climatology_map, fig.asp=0.6, eval=FALSE}
...
```{r dcant_surface_climatology_sections, fig.asp=1, eval=FALSE}
...
```{r surf_equi_truth, fig.asp=1.5, eval=FALSE}
...
```{r surf_equi_truth_bias, fig.asp=1.3, eval=FALSE}
...
d_min <- params_local$depth_mapping

dcant_surface_average <- dcant_surface_average %>% 
  rename(dcant_surface = dcant,
         dcant_pos_surface = dcant_pos) %>% 
  select(data_source, lat, lon, dcant_surface, dcant_pos_surface)

dcant_average <- dcant_average %>% 
  rename(dcant_eMLR = dcant,
         dcant_pos_eMLR = dcant_pos)

# dcant_3d <- full_join(
#   dcant_average_join %>% mutate(method = "eMLR"),
#   dcant_surface_average_join %>% mutate(method = "surface"))
...
dcant_3d <- dcant_3d %>% 
  mutate(dcant_surface = if_else(depth > d_min,
                                 NaN,
                                 dcant_surface),
         dcant_pos_surface = if_else(depth > d_min,
                                 NaN,
                                 dcant_pos_surface))
...
if(d_min > 0) {
  dcant_3d <- dcant_3d %>%
    mutate(
      ...
      dcant_pos = if_else(
        depth <= d_min,
        (mix * dcant_pos_surface) +
          ((1 - mix) * dcant_pos_eMLR),
        dcant_pos_eMLR
      )
    ) %>%
    select(-c(
    ...
    mutate(method = if_else(depth <= d_min,
                            "surface",
                            "eMLR"))
}
...
if(d_min == 0) {
  dcant_3d <- dcant_3d %>%
    mutate(dcant = dcant_eMLR,
           dcant_pos = dcant_pos_eMLR) %>%
    select(-c(
      dcant_eMLR,
      dcant_surface,
      dcant_pos_eMLR,
...
```{r calc_cant_global_mean_sections}

dcant_global_section <- dcant_3d %>%
  group_by(data_source) %>%
  nest() %>%
...
p_section_global(
  df = dcant_global_section %>%
    filter(data_source == "obs"),
  var = "dcant",
  plot_slabs = "n",
  title_text = "Global section basin"
)
...
p_section_global(
  ...
  var = "dcant",
  plot_slabs = "n",
  title_text = "Global section transect"
)
...
```{r calc_dcant_profiles_basin_AIP}
...
```{r check_surface_mapping}
# check if surface data exist
surface_data <- nrow(dcant_3d %>%
...
```{r budget_depth_layer_basin_AIP}
...
dcant_budget_basin_AIP_layer %>%
  ...
  ggplot(aes(value, as.factor(inv_depth), fill = basin_AIP)) +
  scale_fill_brewer(palette = "Dark2") +
  geom_col() +
...
```{r budget_lon_grid_fig, fig.asp=1, eval=surface_data}
...
dcant_budget_lon_grid %>%
  ...
  ggplot(aes(lon_grid, value, fill = method)) +
  scale_fill_brewer(palette = "Dark2") +
  geom_col() +
  facet_grid(estimate ~ data_source)
...
dcant_profile_basin_MLR %>%
  write_csv(paste(path_version_data,
                  "dcant_profile_basin_MLR.csv", sep = ""))
...
dcant_budget_basin_AIP_layer %>%
  write_csv(paste(path_version_data,
                  "dcant_budget_basin_AIP_layer.csv", sep = ""))

dcant_budget_basin_MLR_layer %>%
  write_csv(paste(path_version_data,
                  "dcant_budget_basin_MLR_layer.csv", sep = ""))
...
map +
  geom_raster(data = dcant_3d %>% filter(is.na(depth)),
              aes(lon, lat))
