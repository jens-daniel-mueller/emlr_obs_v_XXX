...
dcant_surface_disequi <- dcant_surface_disequi %>% 
  mutate(pCO2_new = pCO2 + mean_delta_pCO2_atm)
...
dcant_surface_disequi %>%
  mutate(depth = 0,
         delta_rev_fac = rev_fac_new - rev_fac) %>% 
  group_split(data_source) %>%
  # head(1) %>%
  map( ~
         p_map_climatology(
...
# calculate cant
dcant_surface_disequi <- dcant_surface_disequi %>%
  mutate(dcant = (1 / rev_fac_new) *
           (TCO2_new / pCO2_new) * d_pCO2_tref_disequi)
...
full_join(
  dcant_surface %>%
    select(lon, lat, data_source, depth, dcant_surface = dcant),
  dcant_surface_disequi %>%
    select(lon, lat, data_source, depth, dcant_surface_disequi = dcant)
) %>%
  mutate(depth = 0,
         delta_dcant_surface = dcant_surface - dcant_surface_disequi) %>%
  group_split(data_source) %>%
  # head(1) %>%
  map(~
        p_map_climatology(
          df = .x,
          var = "delta_dcant_surface",
          subtitle_text = paste("Data source: ", unique(.x$data_source))
        ))
