...
Preformed DIC was estimated by applying the same DIC/TA ratio as observed at the surface also to deep water layers.

```{r TCO2_TAlk_ratio}

predictors_surface %>% 
  filter(depth %in% c(0,5)) %>% 
  ggplot(aes(TCO2, TAlk)) +
  geom_bin2d() +
...
```{r TCO2_TAlk_climatology_map, fig.asp=0.6}

predictors_surface %>%
  mutate(TCO2_TAlk_ratio = TCO2/TAlk) %>% 
...
```{r calc_TCO2_preformed}

predictors_surface_preformed <- predictors_surface %>%
  filter(depth %in% c(0, 5)) %>%
  mutate(TCO2_TAlk_ratio = TCO2 / TAlk) %>% 
  select(lon, lat, data_source, TCO2_TAlk_ratio)


predictors_surface <- full_join(
  predictors_surface,
  predictors_surface_preformed
)

rm(predictors_surface_preformed)

predictors_surface <- predictors_surface %>% 
  mutate(TCO2_preformed = TAlk * TCO2_TAlk_ratio) %>% 
  select(-TCO2_TAlk_ratio)
...
dcant_surface <- dcant_surface %>%
  mutate(dcant = (1 / rev_fac_preformed) *
           (TCO2_preformed / pCO2_preformed) * d_pCO2_tref)

# calculate positive cant
dcant_surface <- dcant_surface %>%
  mutate(dcant_pos = if_else(dcant < 0, 0, dcant)) %>% 
  select(-c(TCO2_preformed, pCO2_preformed, rev_fac_preformed))
