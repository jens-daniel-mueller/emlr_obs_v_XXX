...
cant_gamma_maps %>%
  ...
  map(
    ~ p_map_cant_slab(
      ...
      geom_point(data = GLODAP_grid %>% filter(gamma_slab == unique(.x$gamma_slab)),
                  aes(lon, lat, col=era_label), size=0.5) +
      scale_color_manual(values = c("grey", "white", "green"))
  )
...
```{r cant_pos_gamma_slab_maps_bias}
