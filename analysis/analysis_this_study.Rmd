...
Global Cant inventories were estimated in units of Pg C. Please note that here we only added positive Cant values in the upper `r params_global$inventory_depth`m and do not apply additional corrections for areas not covered.
...
```{r cant_section_predictor_contribution, eval=FALSE}
...
The predicted target variable (either tco2, cstar or cstar_tref) is based on fitted MLRs and climatological fields of predictor variables, and calculated for each era.
...
target_zonal %>% 
    ggplot(aes(lat, depth,
               z = !!sym(paste(params_local$MLR_target, "mean", sep = "_")))) +
