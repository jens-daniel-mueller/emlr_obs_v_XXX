...
```{r gamma_slab_boundary_depth, fig.asp=1.5}

# gamma field in Atl
predictors_gamma_Atl <- cant_3d %>%
  select(basin_AIP, lat, lon, depth, gamma) %>% 
...
slabs_Atl <- expand_grid(predictors_gamma_Atl %>% distinct(lat, lon),
...
slabs_Ind_Pac <- expand_grid(predictors_gamma_Ind_Pac %>% distinct(lat, lon),
...
```{r calculate_slab_thickness, fig.asp=1.5}
