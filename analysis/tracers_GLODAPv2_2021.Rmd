...
```{r cfc12_Indian_NS_offset, fig.asp=0.4}
...
```{r cfc12_Indian_EW_offset, fig.asp=0.4}
...
p_cfc12_absolut <- zonal_sections %>% 
  ggplot(aes(lat_grid , depth_grid, fill = cfc12)) +
  geom_tile() +
  scale_fill_viridis_c(trans = "log10") +
  scale_y_reverse() +
...
```{r zonal_sections_offset, fig.asp=1}
...
zonal_sections_offset %>% 
  ggplot(aes(delta_cfc12)) +
  geom_histogram() +
  scale_y_log10() +
  coord_cartesian(expand = 0) +
  facet_grid(basin_AIP ~ .)

p_cfc12_offset <- zonal_sections_offset %>% 
  ggplot(aes(lat_grid , depth_grid, fill = delta_cfc12)) +
  geom_raster() +
  scale_fill_divergent(mid = "grey80",
                       na.value = "black",
