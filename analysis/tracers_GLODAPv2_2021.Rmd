...
IO_NS_grid %>% 
  ggplot(aes(lat_grid , depth_grid, fill = pcfc12)) +
  geom_raster() +
  scale_fill_viridis_c() +
  scale_y_reverse() +
...
IO_NS_grid_offset %>% 
  ggplot(aes(lat_grid , depth_grid, fill = delta_pcfc12)) +
  geom_raster() +
...
IO_NS_grid_offset %>% 
  mutate(delta_pcfc12 = cut(delta_pcfc12, c(-Inf, 0.02, 0.05, 0.2, Inf))) %>% 
  ggplot(aes(lat_grid , depth_grid, fill = delta_pcfc12)) +
  geom_raster() +
...
IO_EW_grid %>% 
  ggplot(aes(lon_grid , depth_grid, fill = pcfc12)) +
  geom_tile() +
  scale_fill_viridis_c() +
  scale_y_reverse() +
  coord_cartesian(expand = 0) +
...
IO_EW_grid_offset %>% 
  # drop_na() %>%
  ggplot(aes(lon_grid , depth_grid, fill = delta_pcfc12)) +
  geom_raster() +
  scale_fill_divergent(mid = "grey80",
                       na.value = "black") +
  scale_y_reverse() +
...
unique(dcant_3d$depth)
unique(pCFC_12_3d$depth)
...
```{r dcant_CFC12_correlation_depth, fig.asp=1.5}
...
CFC_dcant %>%
  ...
  geom_smooth(col = "red", se = FALSE) +
  scale_fill_viridis_c(trans = "log10") +
  facet_grid(depth_grid ~ basin_AIP)

```


```{r dcant_CFC12_correlation_depth_gamma, fig.asp=0.5}

CFC_dcant %>%
  group_split(gamma_slab, depth_grid) %>%
  # head(4) %>% 
  map(
    ~ ggplot(data = .x,
             aes(pCFC_12, dcant)) +
      ...
      labs(title = paste("gamma_slab", unique(.x$gamma_slab),
                         "| depth_grid", unique(.x$depth_grid))) +
      facet_grid(. ~ basin_AIP)
  )
