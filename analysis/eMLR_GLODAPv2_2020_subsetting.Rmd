...
GLODAP_combined %>%
  ...
  facet_grid(basin_AIP ~ parameter, scales = "free")
...
cruise_all <- cruise_all %>%
  ...
  mutate(
    ...
    threshold_offset = sd(mean_offset) * 2,
...
ggplot() +
  geom_hline(data = cruise_all, aes(yintercept = c(-1, 1) * threshold_offset)) +
  geom_ribbon(
    data = cruise_all,
    aes(
      x = rank_offset,
...
- `r params_local$A16_GO_SHIP` (y = applied, n = not applied)
