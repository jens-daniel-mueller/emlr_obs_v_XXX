...
Cruises are removed, when the mean offset of the observation from the value predicted with CANYON-B is higher than `r params_local$CANYON_B_max` times the standard deviation of all cruise mean offsets. This criterion is evaluated individually for each variable involved in the eMLR approach. The selection criterion is applied to samples collected deeper than `r params_local$CANYON_B_depth` m.
...
cruise_all <- cruise_all %>%
  group_by(parameter) %>%
  mutate(
    rank_mean_offset = rank(mean_offset),
    mean_limit = sd(mean_offset, na.rm = TRUE) *
      params_local$CANYON_B_max,
    sd_limit = median(sd_offset, na.rm = TRUE) *
      params_local$CANYON_B_max,
    cruise = as.factor(cruise)
  ) %>%
  ungroup() %>%
  arrange(parameter, rank_mean_offset)

cruise_all %>% 
  ggplot() +
  geom_vline(aes(xintercept = c(1,-1)* mean_limit),
             col = "red") +
...
cruise_all %>% 
  ggplot() +
  geom_vline(aes(xintercept = sd_limit),
             col = "red") +
...
cruise_out <- cruise_all %>%
  ...
  filter(abs(mean_offset) > mean_limit)
...
for (i_parameter in unique(cruise_all$parameter)) {
  ...
  print(
    ggplot() +
      geom_hline(
        data = i_cruise_all,
        aes(yintercept = c(-1, 1) * mean_limit),
        lty = 2
      ) +
      geom_ribbon(
        data = i_cruise_all,
      ...
      geom_path(data = i_cruise_all,
                aes(rank_mean_offset, mean_offset)) +
      geom_point(data = i_cruise_out,
                 aes(rank_mean_offset, mean_offset, col = cruise)) +
...
GLODAP_combined <- GLODAP_combined %>%
  filter(!(cruise %in% cruise_out$cruise)) %>% 
  mutate(offset_flag = if_else(abs(offset) <= sd_limit,
                               "included", 
                               "removed"))
