...
cruise_out_adjustment <- GLODAP_adustments %>% 
  filter(if_any(c(tco2_adj, talk_adj), ~ abs(.) > params_local$cruise_adjustment_absolute)) %>% 
...
adjustment_grid %>% 
  ...
  filter(!is.na(adjustment),
         parameter %in% paste0(c("tco2", "talk", "salinity"), "_adj")) %>% 
...
```{r apply_adjustment_filter, eval=FALSE}

GLODAP <- GLODAP %>% 
  filter(!(cruise %in% (cruise_out_adjustment %>% pull())))
