...
GLODAP <- GLODAP %>%
  mutate(
    fill_nitrate = if_else(is.na(nitrate) & oxygenqc == 1,
                           "filled", "measured"),
    fill_phosphate = if_else(is.na(phosphate) & oxygenqc == 1,
                             "filled", "measured"),
    fill_silicate = if_else(is.na(silicate) & oxygenqc == 1,
                            "filled", "measured"),
    fill_talk = if_else(is.na(talk) & oxygenqc == 1,
                        "filled", "measured")
  )

# fill missing values with CANYON-B estimate, if available
GLODAP <- GLODAP %>% 
  mutate(nitrate = if_else(is.na(nitrate) & oxygenqc == 1, 
                           nitrate_CANYONB, nitrate),
         phosphate = if_else(is.na(phosphate) & oxygenqc == 1, 
                             phosphate_CANYONB, phosphate),
         silicate = if_else(is.na(silicate) & oxygenqc == 1, 
                            silicate_CANYONB, silicate),
         talk = if_else(is.na(talk) & oxygenqc == 1, 
                        talk_CANYONB, talk))
