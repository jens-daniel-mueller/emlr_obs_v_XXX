...
cruise_all <- cruise_all %>%
  ...
  mutate(
    ...
    mean_offset_max = sd(mean_offset, na.rm = TRUE) *
      params_local$CANYON_B_max,
    sd_offset_max = median(sd_offset, na.rm = TRUE) *
      params_local$CANYON_B_max,
    cruise = as.factor(cruise)
  ) %>%
  ungroup() %>%
  arrange(parameter, mean_offset_rank)

cruise_all %>% 
  ggplot() +
  geom_vline(aes(xintercept = c(1,-1)* mean_offset_max,
             col = "threshold")) +
  geom_histogram(aes(mean_offset)) +
...
cruise_out <- cruise_all %>%
  ...
  filter(abs(mean_offset) > mean_offset_max)

for (i_parameter in unique(cruise_all$parameter)) {
  # i_parameter <- unique(cruise_all$parameter)[4]
...
if (nrow(GLODAP_out) > 0) {
  map +
    geom_raster(data = GLODAP %>% distinct(lat, lon, era),
                aes(lon, lat, fill = "included")) +
    geom_raster(data = GLODAP_out %>% distinct(lat, lon, era),
                aes(lon, lat, fill = "removed")) +
...
GLODAP_combined <- full_join(GLODAP_combined,
                             parameter_stats)

GLODAP_combined <- GLODAP_combined %>%
  filter(!(cruise %in% cruise_out$cruise)) %>% 
  mutate(offset_flag = if_else(abs(offset) <= sd_offset_max,
                               "included", 
                               "removed"))

GLODAP_combined %>%
  ggplot(aes(offset, fill=offset_flag)) +
  geom_histogram() +
  scale_y_continuous(trans = "log10") +
  facet_grid(basin_AIP ~ parameter, scales = "free_x")

GLODAP_combined %>%
  filter(offset_flag == "removed") %>% 
  ggplot(aes(offset, fill=offset_flag)) +
  geom_histogram() +
  scale_y_continuous() +
  facet_grid(basin_AIP ~ parameter, scales = "free_x")
...
GLODAP_out <- GLODAP %>%
  filter(row_number %in% samples_out$row_number) %>% 
  count(lat, lon, era)
...
nrow(samples_out) / nrow(GLODAP_combined) * 100
