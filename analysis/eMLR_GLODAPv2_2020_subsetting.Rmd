...
cruise_removal_grid <-
  left_join(
    ...
    cruise_all %>% select(cruise, parameter, cruise_flag)
  )
...
samples_removed <- GLODAP_combined %>% 
  filter(sample_flag == "removed") %>% 
  distinct(row_number) %>% 
  pull()

GLODAP <- GLODAP %>% 
  mutate(sample_flag = if_else(row_number %in% samples_removed,
                                "removed",
                                "included"))
