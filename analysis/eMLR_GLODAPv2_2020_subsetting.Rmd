...
GLODAP <- GLODAP %>%
  mutate(
    ...
    fill_phosphate = if_else(
      is.na(phosphate) &
        oxygenqc == 1 &
        cruise_expocode %in% cruises_phosphate_gap_fill,
      "filled",
      "measured"
    ),
...
rm(GLODAP_filling_long, GLODAP_filling_wide, cruises_phosphate_gap_fill)
...
if (0 %in% params_local$flag_f) {
  GLODAP <-
    GLODAP %>%
    filter(if_all(ends_with("f"), ~ . %in% c(2, 9)) |
             cruise_expocode %in% cruises_co2_calc)
} else {
  GLODAP <-
    GLODAP %>%
    filter(if_all(ends_with("f"), ~ . %in% params_local$flag_f))
}

if (0 %in% params_local$flag_qc) {
  GLODAP <-
    GLODAP %>%
    filter(if_all(ends_with("qc"), ~ . %in% c(1)) |
             cruise_expocode %in% cruises_co2_calc)
} else {
  GLODAP <-
    GLODAP %>%
    filter(if_all(ends_with("qc"), ~ . %in% params_local$flag_qc))
}
