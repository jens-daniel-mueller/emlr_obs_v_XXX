...
```{r CANYON_B_offset_map_phosphate, fig.asp=0.6}

i_variable <- "phosphate"
i_offset <- -0.3

map +
  geom_raster(data = GLODAP_combined %>% 
                filter(parameter == i_variable,
                       offset < i_offset),
              aes(lon, lat, fill = as.character(i_offset))) +
  scale_fill_manual(name = paste0("offset threshold:\n", i_variable),
                    values = "red")
...
```{r CANYON_B_offset_map_nitrate, fig.asp=0.6}
...
```{r CANYON_B_offset_map_silicate, fig.asp=0.6}
...
## Manual cruise duplication

```{r Manual_cruise_duplication, fig.asp=2}
...
GLODAP_dup_era1 <- GLODAP %>%
  filter(year >= params_local$dup_era1[1],
         year <= params_local$dup_era1[2]) %>%
  mutate(era = sort(unique(GLODAP$era))[2])

GLODAP_dup_era2 <- GLODAP %>%
  filter(year >= params_local$dup_era2[1],
         year <= params_local$dup_era2[2]) %>%
