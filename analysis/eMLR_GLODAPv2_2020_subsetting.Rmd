...
path_preprocessing_model    <-
  paste(path_root, "/model/preprocessing/GLODAP_subset_A_all_models/", sep = "")
  ...
  GLODAP <-
    read_csv(
      paste0(
        ...
        "GLODAPv2.",
        params_local$GLODAPv2_version,
        "_preprocessed_model_runA_both_NA_filled_ann.csv"
      )
    )
...
# remove obs data from Sea of Japan
GLODAP <- inner_join(GLODAP, basinmask_excl_Japan)
...
```{r CANYON_B_property_property, fig.asp=0.5, eval=FALSE}
...
GLODAP <- left_join(
  GLODAP,
  GLODAP_adjustments %>% distinct(cruise, cruise_expocode)
)
...
GLODAP <- GLODAP %>%
  mutate(
    fill_nitrate = if_else(FALSE,
                           "filled",
                           "measured"),
    fill_phosphate = if_else(
      is.na(phosphate) &
        oxygenqc == 1 &
        cruise_expocode %in% params_local$cruises_phosphate_gap_fill,
      "filled",
      "measured"
    ),
    fill_silicate = if_else(FALSE,
                            "filled",
                            "measured"),
    fill_talk = if_else(
      is.na(talk) &
        oxygenqc == 1 &
        cruise_expocode %in% params_local$cruises_talk_gap_fill,
      "filled",
      "measured"
    )
  )
...
GLODAP <- GLODAP %>%
  mutate(
    ...
    phosphate = if_else(
      fill_phosphate == "filled",
      phosphate_CANYONB * params_local$cruises_phosphate_gap_fill_uncertainty,
      phosphate
    ),
    silicate = if_else(
      fill_silicate == "filled",
      silicate_CANYONB,
      silicate
    ),
    talk = if_else(
      fill_talk == "filled",
      talk_CANYONB + params_local$cruises_talk_gap_fill_uncertainty,
      talk
    )
  )
...
GLODAP_NA <- GLODAP %>% 
  ...
  pivot_longer(c(tco2, talk, params_local$MLR_predictors),
               names_to = "parameter",
               values_to = "value") %>%
...
# Remove all NA data that are still missing despite gap filling
...
cruises_co2_calc <- c(
  params_local$cruises_tco2_calc,
  params_local$cruises_talk_calc
)

# Remove all data that do not meet f-flag criterion,
# except defined exceptions

GLODAP <- GLODAP %>%
  filter(
    if_all(ends_with("f"), ~ . %in% params_local$flag_f) |
      cruise_expocode %in% cruises_co2_calc
  )

# Remove all data that do not meet qc-flag criterion,
# except defined exceptions
...
GLODAP <- GLODAP %>%
  filter(
    if_all(ends_with("qc"), ~ . %in% params_local$flag_qc) |
      cruise_expocode %in% cruises_co2_calc |
      cruise_expocode %in% params_local$cruises_qc_flag |
      cruise_expocode %in% params_local$cruises_talk_gap_fill |
      cruise_expocode %in% params_local$cruises_phosphate_gap_fill
  )
...
```{r cruise_adjustments_histogram_IO, fig.asp=0.5}

left_join(GLODAP %>% distinct(era, cruise, basin_AIP),
            GLODAP_adjustments) %>% 
  filter(
    str_detect(cruise_expocode, "316N199") &
      basin_AIP == "Indian") %>% 
  select(cruise_expocode, tco2_adj, talk_adj, phosphate_adj) %>% 
  kbl()
...
```{r apply_adjustment_filter, fig.asp=1}
...
GLODAP_filling_long  %>%  write_csv(paste(path_version_data,
                                          "GLODAP_CanyonB_filling_stats.csv",
                                          sep = ""))

GLODAP_flagging_stats  %>%  write_csv(paste(path_version_data,
