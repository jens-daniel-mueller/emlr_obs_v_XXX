...
```{r dcant_pos_inventory_map_absolute, fig.asp=0.5}
...
```{r dcant_pos_inventory_map_bias, fig.asp=0.5}
...
```{r dcant_all_inventory_map_absolute, fig.asp=0.5}
...
```{r dcant_all_inventory_map_bias, fig.asp=0.5}
...
```{r dcant_all_inventory_map_bias_decomposition, fig.asp=0.5}

p_map_cant_inv(
    df = target_inv,
    var = "dcant",
    col = "bias",
    subtitle_text = "data_source: delta C* prediction bias")
...
dcant_bias_decomposition <- full_join(
  ...
  target_inv %>%
    select(lat, lon, cstar_predition_bias = dcant) %>%
    drop_na()
)

dcant_bias_decomposition <- dcant_bias_decomposition %>% 
  mutate(dcant_bias = cstar_bias_mod_truth + cstar_predition_bias)
...
p_map_cant_inv(
    ...
    var = "dcant_bias",
    col = "bias",
    subtitle_text = "Combined bias contributions")



```

## Cstar prediction error

```{r cstar_prediction_error, fig.asp=1}
...
```{r dcant_all_inventory_map_bias_cstar}

dcant_inv_cstar %>%
  filter(data_source %in% c("nitrate", "phosphate")) %>% 
  group_split(data_source) %>%
  # head(1) %>%
  map(~ p_map_cant_inv(
    df = .x,
...
## mod_truth vs other Cstar nutrient

```{r dcant_all_inventory_map_bias_cstar_nitrate_phosphate, fig.asp=0.5}

other_cstar_nutrient <-
  c("phosphate", "nitrate")[!(c("phosphate", "nitrate") %in% params_local$cstar_nutrient)]

dcant_inv_bias_cstar <- dcant_inv_cstar %>%
  filter(data_source %in% c("mod_truth", other_cstar_nutrient)) %>%
  select(lat, lon, data_source, dcant) %>%
  pivot_wider(names_from = data_source,
              values_from = dcant) %>%
  mutate(dcant_bias = !!sym(other_cstar_nutrient) - mod_truth) %>%
  drop_na()
...
dcant_inv_bias_cstar <- dcant_inv_cstar %>%
  ...
  pivot_wider(names_from = data_source,
              values_from = dcant) %>%
  mutate(dcant_bias = nitrate - phosphate) %>% 
  drop_na()
...
p_map_cant_inv(
    ...
    var = "dcant_bias",
    col = "bias",
    subtitle_text = "data_source: nitrate - phosphate")
