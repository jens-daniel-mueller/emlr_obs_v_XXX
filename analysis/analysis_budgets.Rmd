...
dcant_budget_global_cstar <- dcant_budget_global_cstar %>%
  mutate(
...
dcant_budget_global_bias_cstar <- dcant_budget_global_cstar %>%
...
dcant_budget_global_bias <-
  bind_rows(
    dcant_budget_global_bias,
    dcant_budget_global_bias_cstar %>%
...
dcant_budget_global_bias %>% 
  ggplot(aes(dcant_bias, estimate, fill=contribution)) +
...
dcant_budget_basin_AIP_cstar <- dcant_budget_basin_AIP_cstar %>%
  mutate(
    data_source = case_when(
      data_source == "po4" ~ "phosphate",
      data_source == "no3" ~ "nitrate",
...
dcant_budget_basin_AIP_bias_cstar <- dcant_budget_basin_AIP_cstar %>%
  filter(data_source %in% c("mod_truth", params_local$cstar_nutrient)) %>%
...
dcant_budget_basin_AIP_bias <-
  bind_rows(
    dcant_budget_basin_AIP_bias,
    dcant_budget_basin_AIP_bias_cstar %>%
      mutate(contribution = "delta C* - mod_truth")
  )
...
```{r dcant_budget_standard_depth_basin_MLR}
...
dcant_budget_basin_MLR_cstar <- dcant_budget_basin_MLR_cstar %>%
  mutate(
    data_source = case_when(
      data_source == "po4" ~ "phosphate",
      data_source == "no3" ~ "nitrate",
...
dcant_budget_basin_MLR_bias_cstar <- dcant_budget_basin_MLR_cstar %>%
  filter(data_source %in% c("mod_truth", params_local$cstar_nutrient)) %>%
...
dcant_budget_basin_MLR_bias <-
  bind_rows(
    dcant_budget_basin_MLR_bias,
    dcant_budget_basin_MLR_bias_cstar %>%
      mutate(contribution = "delta C* - mod_truth")
  )
...
```{r cant_budget_other_depth_basin_MLR, fig.asp=1}
...
dcant_budget_basin_MLR_bias %>% 
  ggplot(aes(dcant_bias_rel, estimate, fill=basin)) +
  geom_vline(xintercept = 0) +
  geom_col(position = "dodge") +
  scale_fill_brewer(palette = "Dark2") +
  facet_grid(inv_depth ~ .)
