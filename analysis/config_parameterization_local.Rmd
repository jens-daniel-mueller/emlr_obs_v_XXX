...
path_preprocessing <-
  "/nfs/kryo/work/jenmueller/emlr_cant/model/preprocessing/GLODAP_subset_A_all_models/"


gobms <- fs::dir_ls(path_preprocessing)

gobms <- gobms[str_detect(gobms, pattern = "_preprocessed_model_runA_both_NA_filled_ann.csv")]

gobms <- str_remove(gobms, path_preprocessing)
gobms <- str_remove(gobms, "_GLODAPv2.2021_preprocessed_model_runA_both_NA_filled_ann.csv")

gobm_select <- gobms[c(1,2,3,5,8)]
# gobm_select <- gobm_select[1]
gobm_select <- "CESM-ETHZ_v20211122_monthly"

rm(path_preprocessing, gobms)
...
params_local <-
  lst(
    ...
    # options: "none", "bulk", "cruise"
    data_adjustment = "bulk",
    
    # bulk corrections for cruises expocode 316N199XXXXX
    cstar_corr_316N199 = 0,
    phosphate_corr_316N199 = 1,
    talk_corr_316N199 = -3.5,
    tco2_corr_316N199 = 1.7,
    
    # bulk corrections for cruises from North Pacific 2010s
    phosphate_corr_NP_2010 = 1.007,
    talk_corr_NP_2010 = 2.05,
    tco2_corr_NP_2010 = -0.6,
    ...
    # ID for basins for MLR fits (options: "1", "2", "AIP", "5", "SO")
    MLR_basins = "AIP",
    ...
    # uncertainty of phosphate gaps filled with CANYON-B
    # cruises_phosphate_gap_fill_uncertainty = 1.01,
    cruises_phosphate_gap_fill_uncertainty = 1,
    
    # cruises for which talk gaps should be filled with CANYON-B
    cruises_talk_gap_fill = c(
      "06AQ19980328"
      ),
    # uncertainty of talk gaps filled with CANYON-B
    # cruises_talk_gap_fill_uncertainty = -1,
    cruises_talk_gap_fill_uncertainty = 0,
    ...
    # nutrient and oxygen predictor fields: WOA18 or GM16
    predictor_climatology = "GM16",
...
# talk_calc_corr = lst(talk_calc_corr = 0)
...
# uncertainty of phosphate gaps filled with CANYON-B
cruises_phosphate_gap_fill_uncertainty = lst(
  # cruises_phosphate_gap_fill_uncertainty = 1.01
  cruises_phosphate_gap_fill_uncertainty = 1
  )
...
# uncertainty of talk gaps filled with CANYON-B
cruises_talk_gap_fill_uncertainty = lst(
  # cruises_talk_gap_fill_uncertainty = -1
  cruises_talk_gap_fill_uncertainty = 0
  )
...
# index of slabs to be plotted
plot_slabs = lst(
  plot_slabs = as.factor(
    c("(26,26.5]", "(27.5,27.75]")
    )
)
...
params_local_all <-
  lst(
    ...
    cruises_phosphate_gap_fill_uncertainty,
    cruises_talk_gap_fill,
    cruises_talk_gap_fill_uncertainty,
