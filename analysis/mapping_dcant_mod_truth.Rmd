...
dcant_3d_cstar <- dcant_3d_cstar %>% 
  ...
  select(lon, lat, depth, basin_AIP, data_source, cstar_variant, method,
...
dcant_3d_cstar %>%
  group_split(cstar_variant) %>%
  # head(1) %>%
  map(~ p_map_climatology(
    df = .x,
    var = "dcant",
    subtitle_text = paste("Cstar_variant: ", .x$cstar_variant)
  ))
