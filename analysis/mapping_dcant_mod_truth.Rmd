...
```{r read_model_dissic_files_simD}
...
```{r calc_model_cant_between_tref_simD}

dcant_3d_D <- left_join(dissic_tref_1, dissic_tref_2) %>%
  mutate(dcant_A = A_2 - A_1,
         dcant_D = D_2 - D_1)

rm(dissic_tref_1, dissic_tref_2)

dcant_3d_D <- dcant_3d_D %>% 
...
dcant_3d_D <- full_join(dcant_3d_D, climatology)

dcant_3d_D <- m_cut_gamma(dcant_3d_D, "gamma")
...
```{r calc_dcant_inventories_simD, fig.asp=1}

dcant_inv_D <- dcant_3d_D %>%
  group_by(data_source) %>%
  nest() %>%
  mutate(inv = map(.x = data, ~ m_dcant_inv(.x))) %>%
  select(-data) %>%
...
```{r budget_global_simD}

dcant_budget_global_D <- m_dcant_budget(dcant_inv_D)

dcant_budget_global_D %>%
  filter(method == "total") %>%
  ggplot(aes(estimate, value)) +
  scale_fill_brewer(palette = "Dark2") +
  geom_col() +
...
```{r budget_basin_AIP_simD}

dcant_budget_basin_AIP_D <- dcant_inv_D %>% 
  group_by(basin_AIP) %>%
  nest() %>% 
  mutate(budget = map(.x = data, ~m_dcant_budget(.x))) %>% 
  select(-data) %>%
...
```{r budget_MLR_regions_simD}

dcant_budget_basin_MLR_D <-
  full_join(dcant_inv_D, basinmask) %>%
  group_by(basin, MLR_basins) %>%
  nest() %>%
  mutate(budget = map(.x = data, ~ m_dcant_budget(.x))) %>%
  select(-data) %>%
